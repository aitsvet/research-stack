...
- **`network_mode: host`** removes the container/host network isolation — every in-container bind lands directly on a host interface. How each service is pinned to loopback:
  ...
  - Open WebUI (`:4096`): `HOST=127.0.0.1`. searxng (`:9090`): `GRANIAN_HOST=127.0.0.1`. mcpo (`:9320`): `--host 127.0.0.1`. open-terminal: `run --host 127.0.0.1`. jupyter-mcp (`:4040`): bind sed-patched after install (upstream hardcodes 0.0.0.0). Chromium CDP (`:9222`) binds loopback by default. Ollama (`:11434`): `OLLAMA_HOST=127.0.0.1:11434` — upstream default is `0.0.0.0:11434`, and host networking means that default would otherwise face the network directly.
