...
| `chandra_ocr.py <pdf> [--out F] [--url U]` | Full-document OCR of a scanned PDF via a vLLM-hosted Chandra model → paginated Markdown (`{N}`+48 dashes). For PDFs with no text layer, where `extract_texts.py md` yields nothing. Needs the endpoint up (`CHANDRA_URL`, default `http://localhost:8000/v1`). |
