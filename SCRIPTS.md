...
| `verify_quotes.py <draft.md> --source <paginated.md> [--page-key printed\|marker]` | Check every quotation in a draft against the page it cites, the way `verify_refs.py` checks DOIs. Quotes are blockquote groups whose last line ends in a page anchor (`— с. 121–122`, `— p. 12`; override with `--anchor`); `[…]`/`…` split a quote into segments that must each appear, so elision is allowed and invention between segments is not. Anchors resolve either by the printed folio (default, a digits-only line — `--folio first` if it sits in the running head) or by the `{N}` mark (`--page-key marker`). Folio lines are stripped before matching so a sentence crossing a page break still matches. Two guards make the result mean something: every anchor in the draft must be claimed by an extracted quote or the run aborts (otherwise the check silently grades a subset), and one verified quote is corrupted and re-checked to prove the comparison can fail. Quotes that also occur on another page are reported as such — the check confirms wording, not that the anchor is the only candidate. |
...
| `split_spreads.py <in.pdf> <out.pdf> [--spine auto\|FRAC] [--single 1,240] [--dry-run]` | Split a PDF imposed **two book pages to a sheet** into one book page per page, so `{N}` marks and the folios printed in the book line up. Purely a /CropBox change (content streams untouched), so it feeds `extract_texts.py md` or `chandra_ocr.py` unchanged instead of duplicating extraction. Rotation goes through the page's own `rotation_matrix`/`derotation_matrix`, so `/Rotate 270` spreads (a portrait book imposed on landscape sheets) split correctly — do not hand-derive the axis mapping. The spine is the widest text-free band near the middle, measured **per line, not per block**: PyMuPDF routinely groups the two running heads of a spread into one block spanning the gutter, which hides the gap and sends every sheet to the 0.5 fallback. The warning that matters is `spine crosses text on N sheet(s)`; the spread of the detected spine across sheets is not itself a defect, since any cut inside an empty gutter is equally good. Covers and plates that are single pages on their own sheet go in `--single`. |
...
| `md_docx.py PAPER.md --template ACCEPTED.docx [--out F] [--image-cm N] [--check]` | Markdown article → journal-formatted `.docx`. Keeps every part of the template (styles, section properties, footnote plumbing, image relationship) and rebuilds only the body. `--check` re-reads the result and diffs its paragraph text against the Markdown. |
...
Language-specific form counts use `participle_patterns` and `participle_ignore`, both lists of
regexes matched against complete words. `participles_per_100` sets the document-level ceiling and
`participles_per_para` reports paragraphs at or above the local ceiling. Keep all four values in the
project config and calibrate them against a venue exemplar; the shared script contains no morphology.
